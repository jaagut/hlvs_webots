...
class Referee:
    def __init__(self):
        ...
        self.config.SIMULATED_TIME_INTERRUPTION_PHASE_0 = int(
            self.config.SIMULATED_TIME_INTERRUPTION_PHASE_0 * 1000 / self.time_step
        )
        self.config.SIMULATED_TIME_BEFORE_PLAY_STATE = int(
            self.config.SIMULATED_TIME_BEFORE_PLAY_STATE * 1000 / self.time_step
        )
        self.config.SIMULATED_TIME_SET_PENALTY_SHOOTOUT = int(
            self.config.SIMULATED_TIME_SET_PENALTY_SHOOTOUT * 1000 / self.time_step
        )
        ...
        self.game_config_file = (
            os.environ["WEBOTS_ROBOCUP_GAME"]
            if "WEBOTS_ROBOCUP_GAME" in os.environ
        ...
        self.forceful_contact_matrix = ForcefulContactMatrix(
            len(self.red_team.players),
            len(self.blue_team.players),
            self.config.FOUL_PUSHING_PERIOD,
            self.config.FOUL_PUSHING_TIME,
            self.time_step,
        )
        ...
        self.game_controller_udp_filter = (
            os.environ["GAME_CONTROLLER_UDP_FILTER"]
            if "GAME_CONTROLLER_UDP_FILTER" in os.environ
            else None
        )
        ...
        try:
            ...
            self.logger.error(
                f"Unexpected exception in main referee loop: {traceback.format_exc()}"
            )
    ...
    def clean_exit(self):
        ...
        if hasattr(self.game, "over") and self.game.over:
            ...
            if (
                hasattr(self.game, "press_a_key_to_terminate")
                ...
                self.logger.info(
                    f"Waiting {waiting_steps} simulation steps before exiting"
                )
    ...
    def print_status(self):
        ...
        now = time.time()
        if (
            self.status_update_last_real_time is None
            or self.status_update_last_sim_time is None
        ):
            ...
            elapsed_simulation = (
                self.sim_time.get_sec() - self.status_update_last_sim_time
            )
            speed_factor = elapsed_simulation / elapsed_real
            messages = [
                f"Avg speed factor: {speed_factor:.3f} (over last {elapsed_real:.2f} seconds)"
            ]
            if self.game.state is None:
                messages.append("No messages received from GameController yet")
            else:
                messages.append(
                    f"state: {self.game.state.game_state}, remaining time: {self.game.state.seconds_remaining}"
                )
                if self.game.state.secondary_state in GAME_INTERRUPTIONS:
                    messages.append(
                        f"  sec_state: {self.game.state.secondary_state} phase: "
    ...
    def toss_a_coin_if_needed(
        self, attribute
    ):  # attribute should be either "side_left" or "kickoff"
        ...
        if hasattr(self.game, attribute):
            if getattr(self.game, attribute) == "red":
                ...
                self.logger.error(
                    f'Unsupported value for "{attribute}" in game.json file: '
            ...
            setattr(self.game, attribute, "random")
        if (
            getattr(self.game, attribute) == "random"
        ):  # toss a coin to determine a random team
            setattr(
                self.game,
                attribute,
                self.game.red.id if bool(random.getrandbits(1)) else self.game.blue.id,
            )
    ...
    def spawn_team(self, team, red_on_right):
        ...
        for number, player in team.players.items():
            ...
            string = (
                f'DEF {def_name} {model}{{name "{color} player {number}" '
            ...
            self.logger.info(
                f"Spawned {def_name} {model} on port {port} at halfTimeStartingPose: translation ("
    ...
    def team_index(self, color):
        ...
        if self.game.state.teams[index].team_number != id:
            raise RuntimeError(
                f"Wrong team number set in team_index(): {id} != {self.game.state.teams[index].team_number}"
            )
        return index
    ...
    def game_controller_receive(self):
        ...
        while True:
            if self.game_controller_udp_filter and ip and ip not in self.others:
                ...
                self.logger.warning(
                    f"Ignoring UDP packets from {ip} not matching "
                    f"GAME_CONTROLLER_UDP_FILTER={self.game_controller_udp_filter}."
                )
            try:
                data, peer = self.game.udp.recvfrom(
                    GameState.sizeof()
                )  # TODO move udp from game.py to referee.py
                ip, port = peer
                if (
                    self.game_controller_udp_filter is None
                    or self.game_controller_udp_filter == ip
                ):
                    break
                else:
                    continue
                ...
                return
        self.previous_seconds_remaining = (
            self.game.state.seconds_remaining if self.game.state else 0
        )
        previous_secondary_seconds_remaining = (
            self.game.state.secondary_seconds_remaining if self.game.state else 0
        )
        previous_state = self.game.state.game_state if self.game.state else None
        previous_sec_state = (
            self.game.state.secondary_state if self.game.state else None
        )
        previous_sec_phase = (
            self.game.state.secondary_state_info[1] if self.game.state else None
        )
        if self.game.state:
        ...
        if previous_state != self.game.state.game_state:
            self.logger.info(
                f"New state received from GameController: {self.game.state.game_state}."
            )
            if self.game.wait_for_state is not None:
                if "STATE_" + self.game.wait_for_state != self.game.state.game_state:
                    self.logger.warning(
                        f"Received unexpected state from GameController: {self.game.state.game_state} "
                        + f"while expecting {self.game.wait_for_state}"
                    )
                else:
                    self.logger.info(
                        f"State has successfully changed to {self.game.wait_for_state}"
                    )
        ...
        if previous_sec_state != new_sec_state or previous_sec_phase != new_sec_phase:
            self.logger.info(
                f"New secondary state received from GameController: {new_sec_state}, phase {new_sec_phase}."
            )
            if (
                self.game.wait_for_sec_state is not None
                or self.game.wait_for_sec_phase is not None
            ):
                if (
                    "STATE_" + self.game.wait_for_sec_state != new_sec_state
                    or new_sec_phase != self.game.wait_for_sec_phase
                ):
                    self.logger.warning(
                        f"Received unexpected secondary state from GameController: "
                        f"{new_sec_state}:{new_sec_phase} "
                        f"while expecting {self.game.wait_for_sec_state}:{self.game.wait_for_sec_phase}"
                    )
                else:
                    self.logger.info(
                        f"State has successfully changed to {new_sec_state}:{new_sec_phase}"
                    )
                    self.game.wait_for_sec_state = None
                    self.game.wait_for_sec_phase = None
        if (
            self.game.state.game_state == "STATE_PLAYING"
            and self.game.state.secondary_seconds_remaining == 0
            and previous_secondary_seconds_remaining > 0
        ):
            if self.game.in_play is None and self.game.phase == "KICKOFF":
                self.logger.info(
                    "Ball in play, can be touched by any player (10 seconds elapsed after kickoff)."
                )
        ...
        if self.previous_seconds_remaining != self.game.state.seconds_remaining:
            allow_in_play = (
                self.game.wait_for_sec_state is None
                and self.game.wait_for_sec_phase is None
            )
            if (
                allow_in_play
                and self.game.state.secondary_state == "STATE_NORMAL"
                and self.game.interruption_seconds is not None
            ):
                if (
                    self.game.interruption_seconds - self.game.state.seconds_remaining
                    > self.config.IN_PLAY_TIMEOUT
                ):
                    if self.game.in_play is None:
                        self.logger.info(
                            "Ball in play, can be touched by any player (10 seconds elapsed)."
                        )
        ...
        if (
            previous_red_score != self.game.state.teams[red].score
            or previous_blue_score != self.game.state.teams[blue].score
        ):
            self.display.update()
        ...
        if (
            secondary_state[0:6] == "STATE_"
            and secondary_state[6:] in GAME_INTERRUPTIONS
        ):
            ...
            if step == 0 and self.game.interruption_step != step:
                ...
                step == 1
                and self.game.interruption_step != step
                and self.game.state.secondary_seconds_remaining <= 0
                and delay >= self.config.SIMULATED_TIME_INTERRUPTION_PHASE_1
            ):
                self.game.interruption_step = step
                ...
                opponent_team = (
                    self.blue_team
                    if secondary_state_info[0] == self.game.red.id
                    else self.red_team
                )
                self.check_team_away_from_ball(
                    opponent_team, self.field.opponent_distance_to_ball
                )
                if kick == "PENALTYKICK":
        ...
        elif secondary_state not in [
            "STATE_NORMAL",
            "STATE_OVERTIME",
            "STATE_PENALTYSHOOT",
        ]:
            print(
                f"GameController {self.game.state.game_state}:{secondary_state}: {secondary_state_info}"
            )
        self.update_penalized()
        if (
            previous_state != self.game.state.game_state
            or previous_sec_state != new_sec_state
            or previous_sec_phase != new_sec_phase
            or previous_secondary_seconds_remaining
            != self.game.state.secondary_seconds_remaining
            or self.game.state.seconds_remaining <= 0
        ):
            self.display.update()
    ...
    def game_controller_send(self, message):
        if (
            ...
            elif message[:6] == "SCORE:" or message == "DROPPEDBALL":
                self.game.wait_for_state = (
                    "FINISHED" if self.game.penalty_shootout else "READY"
                )
            ...
            if msg_start in GAME_INTERRUPTIONS:
                if "ABORT" in message or "EXECUTE" in message:
                    ...
                    if "READY" in message:
                        ...
                        self.game.wait_for_sec_phase = 0
                self.logger.info(
                    f"Waiting for secondary state: {self.game.wait_for_sec_state}:{self.game.wait_for_sec_phase}"
                )
        self.game_controller_send_id += 1
        if message[:6] != "CLOCK:":
            self.logger.info(
                f"Sending {self.game_controller_send_id}:{message} to GameController."
            )
        ...
        while True:
            try:
                answers = (
                    self.game_controller_socket.recv(1024).decode("ascii").split("\n")
                )
                for answer in answers:
                    ...
                    try:
                        ...
                        self.logger.error(
                            f"Received acknowledgment message for unknown message: {id}"
                        )
                        self.clean_exit()
                    if result == "OK":
                        continue
                    if result == "INVALID":
                        self.logger.error(
                            f"Received invalid answer from GameController for message {answered_message}."
                        )
                        self.clean_exit()
                    elif result == "ILLEGAL":
                        ...
                        self.logger.error(
                            f"Received unknown answer from GameController: {answer}."
                        )
                        self.clean_exit()
            except BlockingIOError:
                if answered or ":CLOCK:" in message:
                    ...
                    self.logger.info(
                        f"Waiting for GameController to answer to {message.strip()}."
                    )
                    time.sleep(0.2)
                    self.game_controller_send_id += 1
        ...
        while (
            self.game.wait_for_state is not None
            or self.game.wait_for_sec_state is not None
            or self.game.wait_for_sec_phase is not None
        ):
    ...
    def list_player_solids(self, player, color, number):
        ...
        if len(player["solids"]) != 4:
            ...
            self.logger.error(
                f"{color} player {number}: invalid number of [hand]+[foot], "
    ...
    def update_team_ball_holding(self, team):
        ...
        if goalkeeper_number is not None:  # goalkeeper is in vicinity of ball
            ...
            if aabb_circle_collision(
                aabb,
                self.game.ball_position[0],
                self.game.ball_position[1],
                self.game.ball_radius * self.config.BALL_HOLDING_RATIO,
            ):
                hull = ConvexHull(points)
                hull_vertices = np.take(points, hull.vertices, 0)
                goalkeeper_hold_ball = polygon_circle_collision(
                    hull_vertices,
        ...
        if hasattr(team, "hold_ball"):
            if not hold_ball:
                ...
                self.logger.info(
                    f"{color.capitalize()} team released the ball after {delay} seconds."
                )
                del team.hold_ball
        elif hold_ball:
            team.hold_ball = self.sim_time.get_ms()
            self.logger.info(
                f"{color.capitalize()} team ({numbers}) is holding the ball."
            )

        if hasattr(team, "goalkeeper_hold_ball"):
            if not goalkeeper_hold_ball:
                delay = (
                    int((self.sim_time.get_ms() - team.goalkeeper_hold_ball) / 100) / 10
                )
                self.logger.info(
                    f"{color.capitalize()} goalkeeper released the ball after {delay} seconds."
                )
    ...
    def update_team_contacts(self, team):
        ...
        for number, player in team.players.items():
            ...
            player["velocity_buffer"][
                int(self.sim_time.get_ms() / self.time_step) % l1
            ] = robot.getVelocity()
            sum = [0] * l2
            ...
            if n >= 3:
                player[
                    "outside_circle"
                ] = True  # true if fully outside the center circle
                player["outside_field"] = True  # true if fully outside the field
                player["inside_field"] = True  # true if fully inside the field
                player[
                    "on_outer_line"
                ] = False  # true if robot is partially on the line surrounding the field
                player[
                    "inside_own_side"
                ] = True  # true if fully inside its own side (half field side)
                player[
                    "outside_goal_area"
                ] = True  # true if fully outside of any goal area
                player[
                    "outside_penalty_area"
                ] = True  # true if fully outside of any penalty area
            ...
            for i in range(n):
                ...
                if point[2] > self.field.turf_depth:  # not a contact with the ground
                    if (
                        not early_game_interruption
                        and point in self.ball.contact_points
                    ):  # ball contact
                        if member in ["arm", "hand"]:
                            ...
                            if player["ball_handling_start"] is None:
                                ...
                                self.logger.info(
                                    f"Ball touched the {member} of {color} player {number}."
                                )
                            if (
                                self.game.throw_in
                                and self.game.ball_position[2]
                                > self.field.turf_depth
                                + self.game.ball_radius
                                + self.config.BALL_LIFT_THRESHOLD
                            ):
                        ...
                        if (
                            self.game.penalty_shootout_count >= 10
                        ):  # extended penalty shootout
                            self.game.penalty_shootout_time_to_touch_ball[
                                self.game.penalty_shootout_count - 10
                            ] = (60 - self.game.state.seconds_remaining)
                        if (
                            self.game.ball_last_touch_team != color
                            or self.game.ball_last_touch_player_number != int(number)
                        ):
                            self.game.set_ball_touched(color, int(number))
                            self.game.ball_last_touch_time_for_display = (
                                self.sim_time.get_ms()
                            )
                            action = (
                                "kicked"
                            ...
                            self.logger.info(
                                f"Ball {action} by {color} player {number}."
                            )
                            if self.game.kicking_player_number is None:
                            ...
                            self.logger.info(
                                f"Ball touched again by {color} player {number}."
                            )
                        step = self.game.state.secondary_state_info[1]
                        if (
                            step != 0
                            and self.game.state.secondary_state[6:]
                            in GAME_INTERRUPTIONS
                        ):
                            self.game_interruption_touched(team, number)
                        continue
                    # the robot touched something else than the ball or the ground
                    player["contact_points"].append(
                        point
                    )  # this list will be checked later for robot-robot collisions
                    continue
                ...
                if self.field.point_inside(point):
                    ...
                    if (
                        abs(point[0])
                        > self.field.size_x - self.field.penalty_area_length
                        and abs(point[1]) < self.field.penalty_area_width / 2
                    ):
                        player["outside_penalty_area"] = False
                        if (
                            abs(point[0])
                            > self.field.size_x - self.field.goal_area_length
                            and abs(point[1]) < self.field.goal_area_width / 2
                        ):
                            player["outside_goal_area"] = False
                    if not self.field.point_inside(
                        point, include_turf=False, include_border_line=False
                    ):
                        player["on_outer_line"] = True
                else:
                    player["inside_field"] = False
                if self.game.side_left == (
                    self.game.red.id if color == "red" else self.game.blue.id
                ):
                ...
                if "fallen" in player:  # was already down
                    continue
                self.logger.info(
                    f"{color.capitalize()} player {number} has fallen down."
                )
                player["fallen"] = self.sim_time.get_ms()
            if not player["on_outer_line"]:
                player["on_outer_line"] = not (
                    player["inside_field"] or player["outside_field"]
                )
            if not fallen and "fallen" in player:  # the robot has recovered
                delay = (int((self.sim_time.get_ms() - player["fallen"]) / 100)) / 10
                self.logger.info(
                    f"{color.capitalize()} player {number} just recovered after {delay} seconds."
                )
                del player["fallen"]
            if outside_turf:
        ...
        for number, player in team.players.items():
            ...
            for point in contact_points:
                ...
                if opponent_number is not None:
                    ...
                    if not fcm.contact(
                        red_number, blue_number, self.sim_time.get_ms() - self.time_step
                    ) and not fcm.contact(
                        red_number, blue_number, self.sim_time.get_ms()
                    ):
                        self.logger.info(
                            f"{self.sim_time.get_ms()}: contact between {team.color} player {number} and "
    ...
    def update_histories(self):
        for team in [self.red_team, self.blue_team]:
            for number, player in team.players.items():
                ...
                if len(player["history"]) > 0 and (
                    self.sim_time.get_ms() - player["history"][0][0]
                    > self.config.INACTIVE_GOALKEEPER_TIMEOUT * 1000
                ):
                    player["history"].pop(0)
                # If enough time has elapsed, add an entry
                if (
                    len(player["history"]) == 0
                    or (self.sim_time.get_ms() - player["history"][-1][0])
                    > self.config.BALL_DIST_PERIOD * 1000
                ):
                    ball_dist = distance2(player["position"], self.game.ball_position)
                    own_goal_area = (
                        player["inside_own_side"] and not player["outside_goal_area"]
                    )
                    entry = (
                        self.sim_time.get_ms(),
                        {"ball_distance": ball_dist, "own_goal_area": own_goal_area},
                    )
    ...
    def update_team_penalized(self, team):
        ...
        for number, player in team.players.items():
            ...
            if p.number_of_red_cards > 0:
                ...
                player["robot"] = None
                self.logger.info(
                    f"Sending {color} player {number} to {t}. (team_index: {index})"
                )
    ...
    def forceful_contact_foul(
        self, team, number, opponent_team, opponent_number, distance_to_ball, message
    ):
        ...
        self.logger.info(
            f"{team.color.capitalize()} player {number} committed a forceful contact foul on "
        ...
        immunity_timeout = (
            self.sim_time.get_ms() + self.config.FOUL_PENALTY_IMMUNITY * 1000
        )
        opponent["penalty_immunity"] = immunity_timeout
        player["penalty_immunity"] = immunity_timeout
        freekick_team_id = (
            self.game.blue.id if team.color == "red" else self.game.red.id
        )
        ...
        if self.game.penalty_shootout and self.is_penalty_kicker(team, number):
            self.logger.info(
                f"Kicker {team.color} {number} performed forceful contact during penaltykick -> end of trial"
            )
            self.next_penalty_shootout()
        self.logger.info(
            f"Ball in play: {self.game.in_play}, foul far from ball: {foul_far_from_ball}"
        )
    ...
    def moves_to_ball(self, player, velocity, velocity_squared):
        if (
            velocity_squared
            < self.config.FOUL_SPEED_THRESHOLD * self.config.FOUL_SPEED_THRESHOLD
        ):
            return True
        ...
        angle = math.acos(
            (rx * vx + ry * vy)
            / (math.sqrt(rx * rx + ry * ry) * math.sqrt(vx * vx + vy * vy))
        )
    ...
    def check_team_forceful_contacts(
        self, team, number, opponent_team, opponent_number
    ):
        ...
        debug_messages = [
            f"Check if {p1_str} is performing a foul on {p2_str}",
        ...
        if self.goalkeeper_inside_own_goal_area(opponent_team, opponent_number):
            ...
            self.forceful_contact_foul(
                team, number, opponent_team, opponent_number, d1, "goalkeeper"
            )
            return True
        if team == self.red_team:
            red_number = number
        ...
        if self.forceful_contact_matrix.long_collision(red_number, blue_number):
            if (
                d1 < self.config.FOUL_VICINITY_DISTANCE
                and d1 - d2 > self.config.FOUL_DISTANCE_THRESHOLD
            ):
                collision_time = self.forceful_contact_matrix.get_collision_time(
                    red_number, blue_number
                )
                debug_messages.append(
                    f"Pushing time: {collision_time} > {self.config.FOUL_PUSHING_TIME} "
                    f"over the last {self.config.FOUL_PUSHING_PERIOD}"
                )
                debug_messages.append(
                    f"Difference of distance: {d1-d2} > {self.config.FOUL_DISTANCE_THRESHOLD}"
                )
                self.logger.info(debug_messages)
                self.forceful_contact_foul(
                    team, number, opponent_team, opponent_number, d1, "long_collision"
                )
                return True
        v1 = p1["velocity"]
        v2 = p2["velocity"]
        ...
        if (
            ...
            return False
        debug_messages.append(
            f"{p1_str:6s}: velocity: {self.readable_number_list(v1[:3])}, "
            f"speed: {math.sqrt(v1_squared):.2f}"
        )
        debug_messages.append(
            f"{p2_str:6s}: velocity: {self.readable_number_list(v2[:3])}, "
            f"speed: {math.sqrt(v2_squared):.2f}"
        )
        if d1 < self.config.FOUL_VICINITY_DISTANCE:
            debug_messages.append(
                f"{p1_str} is close to the ball ({d1:.2f} < {self.config.FOUL_VICINITY_DISTANCE})"
            )
            if self.moves_to_ball(p2, v2, v2_squared):
                ...
                if d1 - d2 > self.config.FOUL_DISTANCE_THRESHOLD:
                    debug_messages.append(
                        f"{p2_str} is significantly closer to the ball than {p1_str}: "
            ...
            math.sqrt(v1_squared) - math.sqrt(v2_squared)
            > self.config.FOUL_SPEED_THRESHOLD
        ):
            self.logger.info(debug_messages)
            self.forceful_contact_foul(
                team,
        ...
        for red_number in self.red_team.players:
            for blue_number in self.blue_team.players:
                if not fcm.contact(red_number, blue_number, self.sim_time.get_ms()):
                    continue  # no contact
                if self.check_team_forceful_contacts(
                    self.red_team, red_number, self.blue_team, blue_number
                ):
                    continue
                if self.check_team_forceful_contacts(
                    self.blue_team, blue_number, self.red_team, red_number
                ):
                    continue
    ...
    def check_team_ball_holding(self, team):
        ...
        if sum > size / 2:
            self.logger.info(
                f"{color.capitalize()} team has held the ball for too long."
            )
            return True
        return False

    def check_ball_holding(
        self,
    ):  # return the team id which gets a free kick in case of ball holding from the other team
    ...
    def check_team_ball_handling(self, team):
        for number, player in team.players.items():
            if player["ball_handling_start"] is None:  # ball is not handled
                continue
            duration = (
                player["ball_handling_last"] - player["ball_handling_start"]
            ) / 1000
            color = team.color
            if (
                self.sim_time.get_ms() - player["ball_handling_last"] >= 1000
            ):  # ball was released one second ago or more
                ...
                if self.game.throw_in:
                    ...
                    if was_throw_in and not self.game.throw_in_ball_was_lifted:
                        sentence = (
                            "throw-in with the hand or arm while the ball was not lifted by at least "
            ...
            if not goalkeeper and not self.game.throw_in:
                ...
                if self.game.penalty_shootout and self.is_penalty_kicker(team, number):
                    self.logger.info(
                        "Kicker {color.capitalize()} {number} has fallen down and not recovered -> end of trial"
                    )
                else:
                    self.send_penalty(
                        player,
            ...
            ball_on_the_ground = (
                ...
                <= self.field.turf_depth + self.game.ball_radius
            )
            if self.game.throw_in:
                ...
                if duration >= self.config.BALL_HANDLING_TIMEOUT:
                    self.reset_ball_handling(player)
                    sentence = (
                        f"touched the ball with its hand or arm for more than {self.config.BALL_HANDLING_TIMEOUT} "
            ...
            if duration >= self.config.BALL_HANDLING_TIMEOUT:
                ...
                if (
                    ...
                    and self.sim_time.get_ms() - player["ball_handling_start"] >= 1000
                ):
                    self.reset_ball_handling(player)
                    sentence = (
                        "handled the ball after it received it from teammate"
                        if self.game.ball_last_touch_player_number == int(number)
                        else "released the ball and retook it"
                    )
                    self.logger.info(
                        f"{color.capitalize()} goalkeeper {number} {sentence}."
                    )
                    return True  # a freekick will be awarded
                self.reset_ball_handling(player)
                self.logger.info(
                    f"{color.capitalize()} goalkeeper {number} handled the ball up for more than "
                    f"{self.config.BALL_HANDLING_TIMEOUT} seconds."
                )
                return True  # a freekick will be awarded
            if (
                ball_on_the_ground
                and duration >= self.config.GOALKEEPER_GROUND_BALL_HANDLING
            ):
                self.reset_ball_handling(player)
                self.logger.info(
                    f"{color.capitalize()} goalkeeper {number} handled the ball on the ground for more than "
                    f"{self.config.GOALKEEPER_GROUND_BALL_HANDLING} seconds."
                )
                return True  # a freekick will be awarded
        return False  # not free kick awarded

    def check_ball_handling(
        self,
    ):  # return the team id which gets a free kick in case of wrong ball handling by a goalkeeper
    ...
    def check_team_fallen(self, team):
        ...
        for number, player in team.players.items():
            ...
            if (
                "fallen" in player
                and self.sim_time.get_ms() - player["fallen"]
                > 1000 * self.config.FALLEN_TIMEOUT
            ):
                del player["fallen"]
                if self.game.penalty_shootout and self.is_penalty_kicker(team, number):
                    self.logger.info(
    ...
    def check_team_inactive_goalkeeper(self, team):
        ...
        for number, player in team.players.items():
            ...
            self.logger.info(
                f"Goalkeeper did not move toward the ball over the "
        ...
        for number, player in team.players.items():
            ...
            if d < distance:
                ...
                self.send_penalty(
                    player,
                    "INCAPABLE",
                    f"too close to ball: {d:.2f} m., should be at least {distance:.2f} m."
    ...
    def check_penalty_kick_positions(self):
        for team in [self.red_team, self.blue_team]:
            defending_team = (team.color == "red") ^ (
                self.game.state.secondary_state_info[0] == self.game.red.id
            )
            has_striker = False
            for number, player in team.players.items():
                if self.already_penalized(player):
                ...
                ahead_of_ball = (
                    self.game.ball_position[0] * player_x > 0
                    and abs(player_x) > self.field.penalty_mark_x
                )
                near_ball = (
                    distance2(self.game.ball_position, player["position"])
                    < self.field.opponent_distance_to_ball
                )
                if ahead_of_ball:
                    if not defending_team:
                        ...
                        self.send_penalty(
                            player,
                            "INCAPABLE",
                            "Field player of the attacking team in front of penalty mark",
                        )
                    elif self.is_goalkeeper(team, number):
                        on_goal_line_or_behind = (
                            player["on_outer_line"] or player["outside_field"]
                        ) and abs(player["position"][1]) <= self.config.GOAL_WIDTH
                        if not on_goal_line_or_behind:
                            self.send_penalty(
                                player,
                                "INCAPABLE",
                                "Defending goalkeeper between goal line and penalty mark",
                            )
                    else:
                        self.send_penalty(
                            player,
                    ...
                    if defending_team:
                        self.send_penalty(
                            player,
                            "INCAPABLE",
                            "Player of the defending team near ball (penaltykick)",
                        )
                    elif has_striker:
                        self.send_penalty(
                            player,
                            "INCAPABLE",
                            "Player of the attacking team has already a striker",
                        )
                    else:
                        has_striker = True
                        self.logger.info(
                            f"Player {team.color} {number} is considered as striker"
                        )
    ...
    def check_team_start_position(self, team):
        ...
        for number, player in team.players.items():
            ...
            if not player["outside_field"]:
                self.send_penalty(
                    player, "INCAPABLE", "halfTimeStartingPose inside field"
                )
                penalty = True
            elif not player["inside_own_side"]:
                self.send_penalty(
                    player, "INCAPABLE", "halfTimeStartingPose outside team side"
                )
                penalty = True
        return penalty
        ...
        for number, player in team.players.items():
            ...
            if not player["inside_field"]:
                ...
                self.send_penalty(
                    player,
                    "INCAPABLE",
                    "inside center circle during opponent's kick-off",
                )
                penalty = True
        return penalty
    ...
    def check_team_dropped_ball_position(self, team):
        for number, player in team.players.items():
            ...
            if not player["outside_circle"]:
                self.send_penalty(
                    player, "INCAPABLE", "inside center circle during dropped ball"
                )
            elif not player["inside_own_side"]:
                self.send_penalty(
                    player, "INCAPABLE", "outside team side during dropped ball"
                )
    ...
    def check_team_outside_turf(self, team):
        ...
        for number, player in team.players.items():
            ...
            if player["left_turf_time"] is None:
                continue
            if (
                self.sim_time.get_ms() - player["left_turf_time"]
                < self.config.OUTSIDE_TURF_TIMEOUT * 1000
            ):
                continue
            if self.game.penalty_shootout and self.is_penalty_kicker(team, number):
                self.logger.info(
                    f"Kicker {color.capitalize()} {number} left the field -> end of trial"
                )
                self.next_penalty_shootout()
            else:
                self.send_penalty(
                    player,
                    "INCAPABLE",
                    f"left the field for more than "
    ...
    def check_team_penalized_in_field(self, team):
        ...
        for number, player in team.players.items():
            ...
            self.logger.info(
                f"Penalized {color} player {number} re-entered the field..."
            )
            self.send_penalty(
                player,
                "INCAPABLE",
                "penalized player re-entered field",
        ...
        for number, player in team.players.items():
            ...
            if not player["outside_circle"]:
                ...
                self.send_penalty(
                    player,
                    "INCAPABLE",
                    "entered circle during opponent's kick-off",
    ...
    def check_ball_must_kick(self, team):
        ...
        if (
            self.game.dropped_ball
            or self.game.ball_last_touch_team == self.game.ball_must_kick_team
        ):
        ...
        for number, player in team.players.items():
            ...
            self.send_penalty(
                player,
                "INCAPABLE",
                "non-kicking player touched ball not in play",
    ...
    def is_early_game_interruption(self):
        ...
        """
        return (
            self.is_game_interruption() and self.game.state.secondary_state_info[1] == 0
        )
    ...
    def game_interruption_touched(self, team, number):
        ...
        if opponent:
            ...
            self.game.interruption_countdown = (
                self.config.SIMULATED_TIME_INTERRUPTION_PHASE_0
            )
            self.logger.info(
                f"Ball touched by opponent, retaking {GAME_INTERRUPTIONS[self.game.interruption]}"
            )
            self.logger.info(
                f"Reset interruption_countdown to {self.game.interruption_countdown}"
            )
            self.game_controller_send(
                f"{self.game.interruption}:{self.game.interruption_team}:RETAKE"
            )
        else:
            self.game.in_play = self.sim_time.get_ms()
            self.logger.info(
                f"Ball touched before execute, aborting {GAME_INTERRUPTIONS[self.game.interruption]}"
            )
            self.game_controller_send(
                f"{self.game.interruption}:{self.game.interruption_team}:ABORT"
            )
    ...
    def game_interruption_ball_holding(self, team):
        ...
        if opponent:
            ...
            self.logger.info(
                f"Ball holding by opponent team, retaking {GAME_INTERRUPTIONS[self.game.interruption]}"
            )
            self.logger.info(
                f"Reset interruption_countdown to {self.game.interruption_countdown}"
            )
    ...
    def get_first_available_spot(self, team_color, number, reentry_pos):
        ...
        if not self.is_other_robot_near(
            team_color, number, reentry_pos, self.field.robot_radius
        ):
            return reentry_pos
        ...
        for i in range(1, max_iterations):
            for direction in [preferred_dir, -preferred_dir]:
                ...
                opposite_sides = (
                    current_pos[0] * initial_pos[0] < 0
                )  # current_pos should be on the other side
                out_of_field = abs(current_pos[0]) > self.field.size_x
                if opposite_sides or out_of_field:
                    continue
                if not self.is_other_robot_near(
                    team_color, number, current_pos, self.field.robot_radius
                ):
    ...
    def place_player_at_penalty(self, player, team, number):
        ...
        if (self.game.ball_position[1] > 0 and t[1] > 0) or (
            self.game.ball_position[1] < 0 and t[1] < 0
        ):
            t[1] = -t[1]
            r = rotate_along_z(r)
    ...
    def send_team_penalties(self, team):
        ...
        for number, player in team.players.items():
            if "yellow_card" in player:
                self.game_controller_send(
                    f'CARD:{self.game.red.id if color == "red" else self.game.blue.id}:{number}:YELLOW'
                )
            ...
            if "penalty" in player:
                ...
                player["outside_field"] = True
                self.logger.info(
                    f"{penalty} penalty for {color} player {number}: {reason}."
                )
        ...
        for number, player in team.players.items():
            ...
            if "stabilize" in player:
                if player["stabilize"] == 0:
                    ...
                    robot.resetPhysics()
                    robot.getField("translation").setSFVec3f(
                        player["stabilize_translation"]
                    )
                    robot.getField("rotation").setSFRotation(
                        player["stabilize_rotation"]
                    )
                    del player["stabilize"]
                else:
                    player["stabilize"] -= 1
    ...
    def flip_sides(
        self,
    ):  # flip sides (no need to notify GameController, it does it automatically)
        self.game.side_left = (
            self.game.red.id
            if self.game.side_left == self.game.blue.id
            else self.game.blue.id
        )
        for team in [self.red_team, self.blue_team]:
            for number, player in team.players.items():
                self.flip_poses(player)
    ...
    def reset_player(self, color, number, pose, custom_t=None, custom_r=None):
        ...
        player["position"] = t
        self.logger.info(
            f"{color.capitalize()} player {number} reset to {pose}: "
        ...
        player["enable_actuators_at"] = self.sim_time.get_ms() + int(
            self.config.DISABLE_ACTUATORS_MIN_DURATION * 1000
        )
    ...
    def is_goalkeeper(self, team, id):
        ...
        if (n == self.game.red.id and team == self.red_team) or (
            n == self.game.blue.id and team == self.blue_team
        ):
            index = 0
        else:
            index = 1
    ...
    def penalty_kicker_player(self):
        ...
        attacking_team = (
            self.red_team
            if (self.game.kickoff == self.game.blue.id) ^ default
            else self.blue_team
        )
    ...
    def set_penalty_positions(self):
        ...
        default = self.game.penalty_shootout_count % 2 == 0
        attacking_color = (
            "red" if (self.game.kickoff == self.game.blue.id) ^ default else "blue"
        )
        ...
        for number, player in defending_team.players.items():
            if self.player_has_red_card(player):
                continue
            if (
                self.is_goalkeeper(defending_team, number)
                and self.game.penalty_shootout_count < 10
            ):
        ...
        x = (
            -self.field.penalty_mark_x
            if (self.game.side_left == self.game.kickoff) ^ default
            else self.field.penalty_mark_x
        )
    ...
    def stop_penalty_shootout(self):
        ...
        kickoff_team_leads = (
            kickoff_team.score >= self.game.state.teams[0].score
            and kickoff_team.score >= self.game.state.teams[1].score
        )
        penalty_shootout_count = (
            self.game.penalty_shootout_count % 10
        )  # supports both regular and extended shootout kicks
        if (penalty_shootout_count == 6 and diff == 3) or (
            penalty_shootout_count == 8 and diff == 2
        ):
    ...
    def next_penalty_shootout(self):
        ...
        if (
            not self.game.penalty_shootout_goal
            and self.game.state.game_state[:8] != "FINISHED"
        ):
        ...
        if self.game.penalty_shootout_count == 10:
            self.logger.info(
                "Starting extended penalty shootout without a goalkeeper and goal area entrance allowed."
            )
        ...
        if self.penalty_kicker_player():
            ...
            self.logger.info(
                "Skipping penalty trial because team has no kicker available"
            )
            self.game_controller_send("STATE:SET")
            self.next_penalty_shootout()
        return
    ...
    def check_penalty_goal_line(self):
        ...
        for number, player in defending_team.players.items():
            ignore_player = self.already_penalized(player) or not self.is_goalkeeper(
                defending_team, number
            )
            ...
            if on_goal_line_or_behind and player["inside_own_side"]:
                ...
                if player["invalidGoalkeeperStart"] is None:
                    ...
                    self.sim_time.get_ms() - player["invalidGoalkeeperStart"]
                    > self.config.INVALID_GOALKEEPER_TIMEOUT * 1000
                ):
                    self.logger.info(
                        f"Goalkeeper of team {defending_team.color} is not on goal line "
                        f"since {self.config.INVALID_GOALKEEPER_TIMEOUT} sec"
                    )
                    self.send_penalty(
                        player, "INCAPABLE", "Not on goal line during penalty"
                    )
    ...
    def interruption(
        ...
        team=None,
        location=None,
        is_goalkeeper_ball_manipulation=False,
    ):
        ...
        if interruption_type == "FREEKICK":
            ...
            inside_penalty_area = self.field.circle_fully_inside_penalty_area(
                self.game.ball_position, self.game.ball_radius
            )
            if inside_penalty_area and own_side:
                if is_goalkeeper_ball_manipulation:
                    ...
                    if abs(location[0]) > dx:
                        self.game.ball_kick_translation[0] = dx * (
                            -1 if location[0] < 0 else 1
                        )
                        moved = True
                    if abs(location[1]) > dy:
                        self.game.ball_kick_translation[1] = dy * (
                            -1 if location[1] < 0 else 1
                        )
                        moved = True
                    if moved:
                        self.logger.info(
                            f"Moved the ball on the penalty line at {self.game.ball_kick_translation}"
                        )
                    interruption_type = "INDIRECT_FREEKICK"
                else:
                    interruption_type = "PENALTYKICK"
                    self.game.ball_kick_translation = [
                        self.field.penalty_mark_x,
                        0,
                        0.08,
                    ]
        ...
        self.logger.info(
            f"Interruption countdown set to {self.game.interruption_countdown}"
        )
        if not team:
            self.game.interruption_team = (
                self.game.red.id
                if self.game.ball_last_touch_team == "blue"
                else self.game.blue.id
            )
        else:
            self.game.interruption_team = team
        self.game.ball_must_kick_team = (
            "red" if self.game.interruption_team == self.game.red.id else "blue"
        )
        self.game.reset_ball_touched()
        self.logger.info(
            f"Ball not in play, will be kicked by a player from the {self.game.ball_must_kick_team} team."
        )
        color = "red" if self.game.interruption_team == self.game.red.id else "blue"
        self.logger.info(
            f"{GAME_INTERRUPTIONS[interruption_type].capitalize()} awarded to {color} team."
        )
        self.game_controller_send(
            f"{self.game.interruption}:{self.game.interruption_team}"
        )
    ...
    def throw_in(self, left_side):
        ...
        self.game.ball_kick_translation[1] = sign * (
            self.field.size_y - self.field.line_half_width
        )
    ...
    def corner_kick(self, left_side):
        ...
        sign = -1 if left_side else 1
        self.game.ball_kick_translation[0] = sign * (
            self.field.size_x - self.field.line_half_width
        )
        self.game.ball_kick_translation[1] = (
            self.field.size_y - self.field.line_half_width
    ...
    def dropped_ball(self):
        self.logger.info(
            f"The ball didn't move for the past {self.config.DROPPED_BALL_TIMEOUT} seconds."
        )
        self.game.ball_last_move = self.sim_time.get_ms()
    ...
    def reset_pos_penalized_robots_near(self, position, min_dist):
        for team in [self.red_team, self.blue_team]:
            for number, player in team.players.items():
                if (
                    "penalized" in player
                    and distance2(position, player["position"]) < min_dist
                ):
                    self.place_player_at_penalty(player, team, number)
            ...
            for number, player in team.players.items():
                if "fallen" in player:
                    ...
                    self.send_penalty(
                        player,
                        "INCAPABLE",
                        "Fallen close to ball during GameInterruption",
                    )

    def get_alternative_ball_locations(self, original_pos):
        if (self.game.interruption_team == self.game.red.id) ^ (
            self.game.side_left == self.game.blue.id
        ):
            preferred_x_dir = 1
        else:
            preferred_x_dir = -1
        ...
        offset_x = (
            preferred_x_dir * self.field.place_ball_safety_dist * np.array([1, 0, 0])
        )
        offset_y = (
            preferred_y_dir * self.field.place_ball_safety_dist * np.array([0, 1, 0])
        )
        locations = []
        if (
            self.game.interruption == "DIRECT_FREEKICK"
            or self.game.interruption == "INDIRECT_FREEKICK"
        ):
            # TODO If indirect free kick in opponent penalty area on line parallel to goal line, move it along this line
            for dist_multiplier in range(
                1, self.config.GAME_INTERRUPTION_PLACEMENT_NB_STEPS + 1
            ):
    ...
    def move_robots_away(self, target_location):
        for team in [self.blue_team, self.red_team]:
            for number, player in team.players.items():
                ...
                if (
                    distance2(initial_pos, target_location)
                    < self.field.place_ball_safety_dist
                ):
                    ...
                    for dist_multiplier in range(
                        ...
                        if allowed:
                            ...
                            initial_t = np.array(
                                player["robot"].getField("translation").getSFVec3f()
                            )
                            dst_t = initial_t + diff
                            self.logger.info(
                                f"Moving {team.color} player {number} from {initial_pos} to {pos}"
                            )
                            # Pose of the robot is not changed
                            player["robot"].getField("translation").setSFVec3f(
                                dst_t.tolist()
                            )
    ...
    def game_interruption_place_ball(self, target_location, enforce_distance=True):
        if enforce_distance:
            target_location[
                2
            ] = 0  # Set position along z-axis to 0 for all 'self.field.point_inside' checks
            step = 1
            self.logger.info(f"GI placing ball to {target_location}")
            while step <= 4 and self.is_robot_near(
                target_location, self.field.place_ball_safety_dist
            ):
                if step == 1:
                    self.logger.info("Reset of penalized robots")
                    self.reset_pos_penalized_robots_near(
                        target_location, self.field.place_ball_safety_dist
                    )
                elif step == 2:
                    self.logger.info("Penalizing fallen robots")
                    self.penalize_fallen_robots_near(
                        target_location, self.field.place_ball_safety_dist
                    )
                elif step == 3:
                    ...
                    for loc in self.get_alternative_ball_locations(target_location):
                        ...
                        if self.field.point_inside(loc) and not self.is_robot_near(
                            loc, self.field.place_ball_safety_dist
                        ):
        ...
        self.game.reset_ball_touched()
        self.logger.info(
            f"Ball respawned at {target_location[0]} {target_location[1]} {target_location[2]}."
        )
    ...
    def setup(self):
        ...
        if host != "127.0.0.1" and host != self.game.host:
            self.logger.warning(
                f"Host is not correctly defined in game.json file, "
        ...
        try:
            ...
            try:
                ...
                if not os.path.exists(GAME_CONTROLLER_HOME):
                    self.logger.error(
                        f"{GAME_CONTROLLER_HOME} (GAME_CONTROLLER_HOME) folder not found."
                    )
                    ...
                    path = os.path.join(
                        ...
                        "jar",
                        "config",
                        f"hl_sim_{self.game.field_size}",
                        "teams.cfg",
                    )
                    ...
                    with open(path, "w") as file:
                        file.write(
                            (red_line + blue_line)
                            if self.game.red.id < self.game.blue.id
                            else (blue_line + red_line)
                        )
                    command_line = [
                        os.path.join(JAVA_HOME, "bin", "java"),
                        "-jar",
                        "GameControllerSimulator.jar",
                    ]
                    ...
                    if (
                        hasattr(self.game, "use_bouncing_server")
                        and self.game.use_bouncing_server
                    ):
                        command_line.append("-b")
                        command_line.append(self.game.host)
                        self.udp_bouncer_process = subprocess.Popen(
                            ["python3", "udp_bouncer.py", self.game_config_file]
                        )
                    else:
                        self.udp_bouncer_process = None
                    self.game.controller_process = subprocess.Popen(
                        command_line,
                ...
                self.game.controller_process = None
                self.logger.error(
                    "GAME_CONTROLLER_HOME environment variable not set, unable to launch GameController."
                )
                self.clean_exit()
        except KeyError:
            self.game.controller_process = None
            self.logger.error(
                "JAVA_HOME environment variable not set, unable to launch GameController."
            )
            self.clean_exit()
        ...
        ball_texture = random.choice(
            ["telstar", "teamgeist", "europass", "jabulani", "tango"]
        )
        random.seed()
        children.importMFNodeFromString(
            -1, f'RoboCupBackground {{ texture "{bg}" luminosity {luminosity}}}'
        )
        children.importMFNodeFromString(
            -1, f'RoboCupMainLight {{ texture "{bg}" luminosity {luminosity}}}'
        )
        children.importMFNodeFromString(
            -1, f'RoboCupOffLight {{ texture "{bg}" luminosity {luminosity}}}'
        )
        children.importMFNodeFromString(
            -1, f'RoboCupTopLight {{ texture "{bg}" luminosity {luminosity}}}'
        )
        children.importMFNodeFromString(
            -1, f'RobocupSoccerField {{ size "{self.game.field_size}" }}'
        )
        ...
        players_ball_holding_time_window_size = int(
            1000 * self.config.PLAYERS_BALL_HOLDING_TIMEOUT / self.time_step
        )
        goalkeeper_ball_holding_time_window_size = int(
            1000 * self.config.GOALKEEPER_BALL_HOLDING_TIMEOUT / self.time_step
        )
        self.red_team.players_holding_time_window = np.zeros(
            players_ball_holding_time_window_size, dtype=bool
        )
        self.red_team.goalkeeper_holding_time_window = np.zeros(
            goalkeeper_ball_holding_time_window_size, dtype=bool
        )
        self.blue_team.players_holding_time_window = np.zeros(
            players_ball_holding_time_window_size, dtype=bool
        )
        self.blue_team.goalkeeper_holding_time_window = np.zeros(
            goalkeeper_ball_holding_time_window_size, dtype=bool
        )
        ...
        try:
            if self.game.controller_process:
                self.game_controller_socket = socket.socket(
                ...
                while True:
                    try:
                        ...
                        if retry <= 10:
                            self.logger.warning(
                                f"Could not connect to GameController at localhost:8750: {msg}. "
                                f"Retrying ({retry}/10)..."
                            )
                            time.sleep(
                                retry
                            )  # give some time to allow the GameControllerSimulator to start-up
                            self.supervisor.step(0)
                        else:
                            self.logger.error(
                                "Could not connect to GameController at localhost:8750."
                            )
                            self.game_controller_socket = None
                            self.clean_exit()
                            break
                ...
                try:
                    ...
                    self.logger.error(
                        "Failed to set up UDP socket to listen to GC messages"
                    )
            else:
                self.game_controller_socket = None
        except Exception:
            self.logger.error(
                f"Failed connecting to GameController with the following exception {traceback.format_exc()}"
            )
            self.clean_exit()
        ...
        try:
            ...
            ):  # optional supervisor used for CI tests
                children.importMFNodeFromString(
                    -1,
        ...
        if hasattr(self.game, "record_simulation"):
            try:
                if self.game.record_simulation.endswith(".html"):
                    ...
                    raise RuntimeError(
                        f"Unknown extension for record_simulation: {self.game.record_simulation}"
                    )
            except Exception:
                self.logger.error(
                    f"Failed to start recording with exception: {traceback.format_exc()}"
                )
                self.clean_exit()
    ...
    def main_loop(self):
        ...
        while self.supervisor.step(self.time_step) != -1 and not self.game.over:
            if (
                hasattr(self.game, "max_duration")
                and (time.time() - self.blackboard.start_real_time)
                > self.game.max_duration
            ):
                self.logger.info(
                    f"Interrupting game automatically after {self.game.max_duration} seconds"
                )
                break
            self.print_status()
            ...
            if (
                self.game.state.game_state == "STATE_PLAYING"
                and not self.is_early_game_interruption()
            ):
                ...
                if self.game.in_play is None:
                    ...
                    if (
                        self.game.phase in GAME_INTERRUPTIONS
                        and self.game.state.secondary_state[6:] == "NORMAL"
                    ):
                        opponent_team = (
                            self.red_team
                            if self.game.ball_must_kick_team == "blue"
                            else self.blue_team
                        )
                        self.check_team_away_from_ball(
                            opponent_team, self.field.opponent_distance_to_ball
                    ...
                    if (
                        ...
                        and self.game.kickoff != self.config.DROPPED_BALL_TEAM_ID
                    ):
                        defending_team = (
                            self.red_team
                            if self.game.kickoff == self.game.blue.id
                            else self.blue_team
                        )
                        self.check_circle_entrance(defending_team)
                    if self.game.ball_first_touch_time != 0:
                        d = distance2(
                            self.game.ball_kick_translation, self.game.ball_position
                        )
                        if d > self.config.BALL_IN_PLAY_MOVE:
                            self.logger.info(
                                f"{self.game.ball_kick_translation} {self.game.ball_position}"
                            )
                            self.logger.info(
                                f"Ball in play, can be touched by any player (moved by {d * 100:.2f} cm)."
                            )
                            self.game.in_play = self.sim_time.get_ms()
                        elif (
                            not self.is_game_interruption()
                        ):  # The game interruption case is handled in update_team_contacts
                            team = (
                                self.red_team
                                if self.game.ball_must_kick_team == "blue"
                                else self.blue_team
                    ...
                    if (
                        self.sim_time.get_ms() - self.game.ball_last_move
                        > self.config.DROPPED_BALL_TIMEOUT * 1000
                    ):
                        self.dropped_ball()
                    if (
                        self.game.ball_left_circle is None
                        and self.game.phase == "KICKOFF"
                    ):
                        if (
                            distance2(
                                self.game.ball_kick_translation, self.game.ball_position
                            ...
                            self.logger.info(
                                "The ball has left the center circle after kick-off."
                            )

                    ball_touched_by_opponent = (
                        self.game.ball_last_touch_team
                        and self.game.ball_last_touch_team
                        != self.game.ball_must_kick_team
                    )
                    ball_touched_by_teammate = (
                        self.game.kicking_player_number is not None
                        and self.game.ball_last_touch_player_number
                        != self.game.kicking_player_number
                    )
                    ball_touched_in_play = (
                        self.game.in_play is not None
                        and self.game.in_play < self.game.ball_last_touch_time
                    )
                    if not self.game.can_score:
                        if self.game.phase == "KICKOFF":
                            ball_touched_after_leaving_the_circle = (
                                self.game.ball_left_circle is not None
                                and self.game.ball_left_circle
                                < self.game.ball_last_touch_time
                            )
                            if (
                                ball_touched_by_opponent
                                or ball_touched_by_teammate
                                or ball_touched_after_leaving_the_circle
                            ):
                                self.game.can_score = True
                        elif self.game.phase == "THROWIN":
                            if (
                                ball_touched_by_teammate
                                or ball_touched_by_opponent
                                or ball_touched_in_play
                            ):
                                self.game.can_score = True
                ...
                if self.game.penalty_shootout:
                    ...
                    ball_in_goal_area = self.field.circle_fully_inside_goal_area(
                        self.game.ball_position, self.game.ball_radius
                    )
                    ...
                    if (
                        ball_in_goal_area
                        and np.linalg.norm(ball_vel) < self.config.STATIC_SPEED_EPS
                    ):
                        self.logger.info(
                            f"Ball stopped in goal area at {self.game.ball_position}"
                        )
                        self.next_penalty_shootout()
                    if (
                        self.game.penalty_shootout_count < 10
                    ):  # detect entrance of kicker in the goal area
                        kicker = self.penalty_kicker_player()
                        if kicker is None or (
                            not kicker["outside_goal_area"]
                            and not kicker["inside_own_side"]
                        ):
                        ...
                        if ball_in_goal_area:
                            ...
                            if (
                                self.game.penalty_shootout_time_to_reach_goal_area[c]
                                is None
                            ):
                                self.game.penalty_shootout_time_to_reach_goal_area[
                                    c
                                ] = (60 - self.game.state.seconds_remaining)
                if self.previous_seconds_remaining != self.game.state.seconds_remaining:
                    self.display.update()
                    ...
                    if (
                        self.game.state.game_state != "STATE_FINISHED"
                        and self.game.state.seconds_remaining <= 0
                        and not self.game.state.secondary_state == "PENALTYKICK"
                    ):
                        self.logger.info(
                            f"Sending FINISH because seconds remaining = {self.game.state.seconds_remaining}"
                        )
                        self.game_controller_send("STATE:FINISH")
                        if self.game.penalty_shootout:  # penalty timeout was reached
                            ...
                            game_type = (
                                "knockout "
                                if self.game.type == "KNOCKOUT" and self.game.overtime
                                else ""
                            )
                            ...
                            self.logger.error(
                                f"Unsupported game type: {self.game.type}."
                            )
                            self.clean_exit()
                if (
                    self.game.interruption_countdown == 0
                    and self.game.ready_countdown == 0
                    and self.game.ready_real_time is None
                    and not self.game.throw_in
                    and (
                        self.game.ball_position[1] - self.game.ball_radius
                        >= self.field.size_y
                        or self.game.ball_position[1] + self.game.ball_radius
                        <= -self.field.size_y
                        or self.game.ball_position[0] - self.game.ball_radius
                        >= self.field.size_x
                        or self.game.ball_position[0] + self.game.ball_radius
                        <= -self.field.size_x
                    )
                    ...
                    self.logger.info(
                        f"Ball left the field at ({self.game.ball_position[0]} {self.game.ball_position[1]} "
                    ...
                    right_way = None
                    if (
                        self.game.ball_exit_translation[1] - self.game.ball_radius
                        > self.field.size_y
                    ):
                        if self.game.penalty_shootout:
                            self.next_penalty_shootout()
                        else:
                            self.game.ball_exit_translation[1] = (
                                self.field.size_y - self.field.line_half_width
                            )
                    ...
                    if (
                        self.game.ball_exit_translation[0] - self.game.ball_radius
                        > self.field.size_x
                    ):
                        right_way = (
                            self.game.ball_last_touch_team == "red"
                            and self.game.side_left == self.game.red.id
                            or self.game.ball_last_touch_team == "blue"
                            and self.game.side_left == self.game.blue.id
                        )
                        if (
                            self.config.GOAL_HALF_WIDTH
                            > self.game.ball_exit_translation[1]
                            > -self.config.GOAL_HALF_WIDTH
                            and self.game.ball_exit_translation[2]
                        ...
                        if (
                            ...
                            scoring_team = (
                                self.game.red.id
                                if self.game.blue.id == self.game.side_left
                                else self.game.blue.id
                            )
                        elif self.game.penalty_shootout:
                            self.next_penalty_shootout()
                    ...
                    if scoring_team:
                        ...
                        if (
                            self.game.penalty_shootout_count >= 10
                        ):  # extended penalty shootouts
                            extended_penalty_idx = self.game.penalty_shootout_count - 10
                            self.game.penalty_shootout_time_to_score[
                                extended_penalty_idx
                            ] = (60 - self.game.state.seconds_remaining)
                        if not self.game.penalty_shootout:
                            self.game.kickoff = (
                                self.game.blue.id
                                if scoring_team == self.game.red.id
                                else self.game.red.id
                            )
                        ...
                        if not self.game.can_score:
                            if self.game.phase == "KICKOFF":
                                self.logger.info(
                                    f"Invalidated direct score in {goal} goal from kick-off position."
                                )
                                self.goal_kick()
                            elif self.game.phase in GAME_INTERRUPTIONS:
                                self.logger.info(
                                    f"Invalidated direct score in {goal} goal from "
                                    f"{GAME_INTERRUPTIONS[self.game.phase]}."
                                )
                                if not right_way:  # own_goal
                                    self.corner_kick(
                                        left_side=scoring_team != self.game.side_left
                                    )
                                else:
                                    self.goal_kick()
                            ...
                            self.game.ball_last_touch_player_number is not None
                            and self.game.state.teams[i]
                            .players[self.game.ball_last_touch_player_number - 1]
                            .secs_till_unpenalized
                            == 0
                        ):
                            self.game_controller_send(f"SCORE:{scoring_team}")
                            self.logger.info(
                                f"Score in {goal} goal by {self.game.ball_last_touch_team} "
                                f"player {self.game.ball_last_touch_player_number}"
                            )
                            if self.game.penalty_shootout:
                                ...
                                self.game.ready_countdown = (
                                    self.config.SIMULATED_TIME_INTERRUPTION_PHASE_0
                                )
                                self.logger.info(
                                    f"Ready countdown was set to {self.game.ready_countdown}"
                                )
                                self.kickoff()
                        elif not right_way:  # own goal
                            self.game_controller_send(f"SCORE:{scoring_team}")
                            self.logger.info(
                                f"Score in {goal} goal by {self.game.ball_last_touch_team} player "
                            ...
                            self.logger.info(
                                f"Invalidated score in {goal} goal by penalized {self.game.ball_last_touch_team} "
                                f"player {self.game.ball_last_touch_player_number}"
                            )
                            if self.game.penalty_shootout:
                                self.next_penalty_shootout()
                ...
                if self.game.play_countdown == 0:
                    ...
                    if self.game.ball_set_kick:
                        self.game_interruption_place_ball(
                            self.game.ball_kick_translation, enforce_distance=False
                        )
                else:
                    if self.game.penalty_shootout:
                        self.check_penalty_goal_line()
                ...
                if self.game.penalty_shootout:
                    ...
                    if self.game.ready_real_time is None:
                        if (
                            ...
                            self.logger.info(
                                f"Going to SET in {self.config.HALF_TIME_BREAK_REAL_TIME_DURATION} "
                                f"seconds (real-time)"
                            )
                            self.game.set_real_time = (
                                time.time()
                                + self.config.HALF_TIME_BREAK_REAL_TIME_DURATION
                            )
                            ...
                            self.logger.info(
                                f"Going to READY in {self.config.HALF_TIME_BREAK_REAL_TIME_DURATION} "
                                f"seconds (real-time)"
                            )
                            self.game.ready_real_time = (
                                time.time()
                ...
                if self.game.penalty_shootout:
                    ...
                    if (
                        self.game.ready_countdown == 0
                    ):  # kick-off after goal or dropped ball
                        self.check_start_position()
                        self.game_controller_send("STATE:READY")
                    ...
                    self.kickoff()
                    self.logger.info(
                        f"Going to READY in {self.config.HALF_TIME_BREAK_REAL_TIME_DURATION} seconds (real-time)"
                    )
                    self.game.ready_real_time = (
                        time.time() + self.config.HALF_TIME_BREAK_REAL_TIME_DURATION
                    )
            ...
            if self.game.interruption_countdown > 0:
                ...
                if self.game.interruption_countdown == 0:
                    if self.game.ball_set_kick:
                        self.game_interruption_place_ball(
                            self.game.ball_kick_translation, enforce_distance=True
                        )
                    if self.game.interruption:
                        self.game_controller_send(
                            f"{self.game.interruption}:{self.game.interruption_team}:READY"
                        )
            ...
            if self.game.state.game_state == "STATE_PLAYING" and self.game.in_play:
                if not self.game.penalty_shootout:
                    ball_holding = (
                        self.check_ball_holding()
                    )  # check for ball holding fouls
                    if ball_holding:
                        if self.is_game_interruption():
                            self.game_interruption_ball_holding(ball_holding)
                        else:
                            self.interruption(
                                "FREEKICK", ball_holding, self.game.ball_position
                            )
                ball_handling = (
                    self.check_ball_handling()
                )  # return team id if ball handling is performed by goalkeeper
                if ball_handling and not self.game.penalty_shootout:
                    ...
                    self.interruption(
                        ...
                        ball_handling,
                        self.game.ball_position,
                        is_goalkeeper_ball_manipulation=True,
                    )
            self.check_penalized_in_field()  # check for penalized robots inside the field
            if (
                self.game.state.game_state != "STATE_INITIAL"
            ...
            if self.game.minimum_real_time_factor != 0:
                # slow down the simulation to guarantee a minimum amount of real time between each step
                t = time.time()
                delta_time = (
                    previous_real_time
                    - t
                    + self.game.minimum_real_time_factor * self.time_step / 1000
                )
                if delta_time > 0:
        ...
        if not self.game.over:
            ...
            if self.game.state.teams[0].score > self.game.state.teams[1].score:
                ...
                loser = 0
            self.logger.info(
                f"The score is {self.game.state.teams[winner].score}-{self.game.state.teams[loser].score}."
            )
            if self.game.state.teams[0].score != self.game.state.teams[1].score:
                self.logger.info(
                    f"The winner is the {self.game.state.teams[winner].team_color.lower()} team."
                )
                ...
                for i in range(5):
                    if (
                        self.game.penalty_shootout_time_to_reach_goal_area[2 * i]
                        is not None
                    ):
                        count[0] += 1
                    if (
                        self.game.penalty_shootout_time_to_reach_goal_area[2 * i + 1]
                        is not None
                    ):
                        count[1] += 1
                if self.game.kickoff == self.game.red.id:
                    ...
                    count_blue = count[0]
                self.logger.info(
                    "The during the extended penalty shootout, the ball reached "
                ...
                if count_red > count_blue:
                    ...
                    for i in range(5):
                        if (
                            self.game.penalty_shootout_time_to_touch_ball[2 * i]
                            is not None
                        ):
                            count[0] += 1
                        if (
                            self.game.penalty_shootout_time_to_touch_ball[2 * i + 1]
                            is not None
                        ):
                            count[1] += 1
                    if self.game.kickoff == self.game.red.id:
                        ...
                        count_blue = count[0]
                    self.logger.info(
                        f"The ball was touched {count_red} times by the red player "
                    ...
                    if count_red > count_blue:
                        ...
                        if self.game.kickoff == self.game.red.id:
                            ...
                            sum_blue = sum[0]
                        self.logger.info(
                            f"The red team took {sum_red} seconds to score "
                        ...
                        if sum_blue < sum_red:
                            ...
                            sum = [0, 0]
                            for i in range(5):
                                t = self.game.penalty_shootout_time_to_reach_goal_area[
                                    2 * i
                                ]
                                sum[0] += 60 if t is None else t
                                t = self.game.penalty_shootout_time_to_reach_goal_area[
                            ...
                            if sum_blue < sum_red:
                                ...
                                sum = [0, 0]
                                for i in range(5):
                                    t = self.game.penalty_shootout_time_to_touch_ball[
                                        2 * i
                                    ]
                                    sum[0] += 60 if t is None else t
                                    t = self.game.penalty_shootout_time_to_touch_ball[
                                ...
                                if self.game.kickoff == self.game.red.id:
                                    ...
                                    sum_blue = sum[0]
                                self.logger.info(
                                    f"The red team took {sum_red} seconds to touch the ball "
                                    + f"while blue team took {sum_blue} seconds."
                                )
                                if sum_blue < sum_red:
                                    self.logger.info("The winner is the blue team.")
                                    ...
                                    self.logger.info(
                                        "Tossing a coin to determine the winner."
                                    )
